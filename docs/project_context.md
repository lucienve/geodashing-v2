...
- Upgraded the default AI summary model to the next-generation 3.x series (**`gemini-3.5-pro`**), fully parameterized alongside `GEMINI_API_KEY` under the new `[gemini]` section of `config.ini` (with a complete template supplied in `config.ini.example`).
