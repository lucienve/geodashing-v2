...
GEMINI_MODEL = "gemini-3.5-flash"  # The candidate model being validated
GEMINI_PROJECT_ID = "your-gcp-project-id"  # For project-level quota billing
GEMINI_EVAL_MODEL = "gemini-3.5-flash"  # The locked judge model (optional)
```
...
A separate judge model (defined by `GEMINI_EVAL_MODEL` or falling back to `gemini-3.5-flash`) evaluates the generated summary against the raw input text. The judge returns a structured JSON object scoring three criteria from 1 to 5:
