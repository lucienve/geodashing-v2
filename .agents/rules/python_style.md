...
- **Linting:** Automatically invoke `pylint` against any modified Python files and strictly resolve any lint warnings, unused imports, or trailing whitespaces before returning the code. Strongly prefer explicitly refactoring code (e.g. splitting into smaller helper functions) to solve lint issues over simply adding `# pylint: disable=` ignores.
- **Documentation:** Use Google-style docstrings for all modules, classes, and exposed functions.
- **Testing:** Use `pytest` for python. Test core logic rigorously. When utilizing `unittest.mock` for mocking dependencies, always prefer enforcing strict object interfaces via the `spec` or `autospec` arguments to prevent mock drift.
