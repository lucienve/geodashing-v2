...
def test_get_db_connection_file_not_found() -> None:
    ...
    with unittest.mock.patch('os.path.exists',
        ...
        with pytest.raises(FileNotFoundError,
                           match="Database config not found"):
...
def test_get_db_connection_success() -> None:
    ...
    with unittest.mock.patch('os.path.exists', return_value=True, autospec=True), \
        ...
        mock_db_section = unittest.mock.MagicMock(
            spec=configparser.SectionProxy)
        ...
        mock_connect.assert_called_once_with(host='127.0.0.1',
                                             user='geodashing',
                                             password='',
                                             database='geodashing',
                                             port='3306')
...
def test_get_db_connection_mysql_error() -> None:
    ...
    with unittest.mock.patch('os.path.exists', return_value=True, autospec=True), \
        ...
        with pytest.raises(RuntimeError,
                           match="Database Connection Error: Mock DB Error"):
            backend.scripts.db_utils.get_db_connection("fake/path.ini")
...
def test_db_session_success() -> None:
    ...
    with unittest.mock.patch('backend.scripts.db_utils.get_db_connection',
                             return_value=mock_conn,
                             autospec=True) as mock_get_conn:
        with backend.scripts.db_utils.db_session("fake/path.ini") as (conn,
                                                                      cursor):
            assert conn == mock_conn
