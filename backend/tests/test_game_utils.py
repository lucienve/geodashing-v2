...
class TestHTMLFragmentValidator(unittest.TestCase):
    ...
    def test_forbidden_script_tags_fail(self) -> None:
        ...
        self.assertTrue(
            any("Forbidden tag" in err and "script" in err for err in errors1))

        errors2 = self.validator.validate(bad_html_2)
        self.assertTrue(
            any("Forbidden tag" in err and "iframe" in err for err in errors2))
    ...
    def test_disallowed_tag_fail(self) -> None:
        ...
        self.assertTrue(
            any("Disallowed tag detected: <marquee>" in err for err in errors))
    ...
    def test_mismatched_and_unclosed_tags(self) -> None:
        ...
        self.assertTrue(any("Mismatched closing tag" in err
                            for err in errors1))

        errors2 = self.validator.validate(unclosed)
        self.assertTrue(any("Unclosed tags detected" in err
                            for err in errors2))
    ...
    def test_missing_and_invalid_attributes(self) -> None:
        ...
        self.assertTrue(
            any("Anchor <a> tag is missing href attribute" in err
                for err in errors1))
        ...
        self.assertTrue(
            any("Image <img> tag is missing src attribute" in err
                for err in errors4))
...
class TestUploadSummaryCLI(unittest.TestCase):
    ...
    @unittest.mock.patch("backend.scripts.game_utils.os.path.exists",
                         autospec=True)
    @unittest.mock.patch("builtins.open",
                         new_callable=unittest.mock.mock_open,
                         read_data="<p>Awesome summary content</p>")
    def test_successful_summary_upload(
            self, mock_file_open: unittest.mock.MagicMock,
            mock_path_exists: unittest.mock.MagicMock) -> None:
        ...
        mock_conn = unittest.mock.MagicMock(
            spec=mysql.connector.connection.MySQLConnection)

        # Execute upload
        backend.scripts.game_utils.upload_summary(mock_cursor, mock_conn, 12,
                                                  "dummy_summary.html")

        # Verify DB queries
        mock_cursor.execute.assert_any_call(
            "SELECT id, title FROM games WHERE id = %s", (12, ))
        mock_cursor.execute.assert_any_call(
        ...
        mock_file_open.assert_called_once_with("dummy_summary.html",
                                               'r',
                                               encoding='utf-8')

    @unittest.mock.patch("backend.scripts.game_utils.os.path.exists",
                         autospec=True)
    def test_non_existent_game_id_aborts(
            self, mock_path_exists: unittest.mock.MagicMock) -> None:
        ...
        with self.assertRaises(SystemExit):
            backend.scripts.game_utils.upload_summary(mock_cursor, mock_conn,
                                                      999, "dummy.html")
    ...
    def test_invalid_summary_fails_upload(
        ...
        mock_file_open.assert_called_once_with("bad.html",
                                               'r',
                                               encoding='utf-8')
        mock_conn.commit.assert_not_called()
...
class TestEmailSummaryCLI(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.mock_cursor = unittest.mock.MagicMock(
            spec=mysql.connector.cursor.MySQLCursor)
        self.mock_conn = unittest.mock.MagicMock(
            spec=mysql.connector.connection.MySQLConnection)

    @unittest.mock.patch("backend.scripts.game_utils.os.path.exists",
                         autospec=True)
    def test_missing_game_id_aborts(
            self, mock_path_exists: unittest.mock.MagicMock) -> None:
        ...
        with self.assertRaises(SystemExit):
            backend.scripts.game_utils.email_summary(self.mock_cursor, 999,
                                                     "dummy_config.ini")

    @unittest.mock.patch("backend.scripts.game_utils.os.path.exists",
                         autospec=True)
    def test_missing_summary_aborts(
            self, mock_path_exists: unittest.mock.MagicMock) -> None:
        """If the game exists but has no summary, the script should terminate with SystemExit."""
        mock_path_exists.return_value = True
        self.mock_cursor.fetchone.return_value = ("Game 12", None,
                                                  datetime.datetime(
                                                      2026, 5, 1, 0, 0))

        with self.assertRaises(SystemExit):
            backend.scripts.game_utils.email_summary(self.mock_cursor, 12,
                                                     "dummy_config.ini")

    @unittest.mock.patch("backend.scripts.game_utils.os.path.exists",
                         autospec=True)
    def test_testing_env_bypass(
            self, mock_path_exists: unittest.mock.MagicMock) -> None:
        """If APP_ENV is 'testing', physical email sending must be bypassed and exit cleanly."""
        mock_path_exists.return_value = True
        self.mock_cursor.fetchone.return_value = ("Game 12",
                                                  "<p>Summary html</p>",
                                                  datetime.datetime(
                                                      2026, 5, 1, 0, 0))
    ...
    @unittest.mock.patch("backend.scripts.game_utils.os.path.exists",
                         autospec=True)
    def test_successful_email_dispatch(
            self, mock_path_exists: unittest.mock.MagicMock) -> None:
        ...
        mock_creds = unittest.mock.MagicMock(
            spec=google.oauth2.service_account.Credentials)
        ...
        with unittest.mock.patch("builtins.open", unittest.mock.mock_open(read_data=config_data)), \
            ...
            mock_creds.with_subject.assert_called_once_with(
...
class TestRolloverCLI(unittest.TestCase):
    ...
    def test_load_game_config_defaults(self) -> None:
        ...
        with unittest.mock.patch("backend.scripts.game_utils.os.path.exists",
                                 return_value=False):
            config = backend.scripts.game_utils.load_game_config(
                "nonexistent.ini")
    ...
    def test_get_game_title_from_json(self) -> None:
        ...
        titles_data = json.dumps({
            ...
            "2026-08":
            "Briars, Bogs, and Other Minor Inconveniences"
        })
        with unittest.mock.patch("backend.scripts.game_utils.os.path.exists", return_value=True), \
             unittest.mock.patch("builtins.open", unittest.mock.mock_open(read_data=titles_data)):
            title = backend.scripts.game_utils.get_game_title(
                2026, 8, "data/game_titles.json")
    ...
    def test_get_games_for_month(self) -> None:
        ...
        games = backend.scripts.game_utils.get_games_for_month(
            self.mock_cursor, 2026, 6)
    ...
    def test_activate_current_month_success(self) -> None:
        ...
        self.mock_cursor.fetchall.return_value = [(14, "August 2026", False,
                                                   None, None)]
    ...
    def test_generate_next_preview_game_already_exists(self) -> None:
        ...
        self.mock_cursor.fetchall.return_value = [(16, "September 2026", False,
                                                   None, None)]
    ...
    @unittest.mock.patch(
        "backend.scripts.generate_game.generate_valid_dashpoints")
    ...
    def test_generate_next_preview_game_generates(
            self, mock_bulk_insert: unittest.mock.MagicMock,
            mock_init_game: unittest.mock.MagicMock,
            mock_gen_points: unittest.mock.MagicMock) -> None:
        ...
        self.assertEqual(preview_title,
                         "Random Destinations of Unwarranted Grandeur")
    ...
    @unittest.mock.patch(
        "backend.scripts.game_utils._activate_current_month_game")
    @unittest.mock.patch(
        "backend.scripts.game_utils._generate_next_preview_game")
    @unittest.mock.patch(
        "backend.scripts.game_utils._dispatch_announcement_email")
    def test_execute_rollover_full_flow(
            self, mock_dispatch: unittest.mock.MagicMock,
            mock_gen_preview: unittest.mock.MagicMock,
            mock_activate: unittest.mock.MagicMock,
            mock_load_cfg: unittest.mock.MagicMock) -> None:
        ...
        backend.scripts.game_utils.execute_rollover(self.mock_cursor,
                                                    self.mock_conn,
                                                    "dummy.ini",
                                                    dry_run=False,
                                                    overrides={
        ...
        mock_activate.assert_called_once_with(self.mock_cursor, self.mock_conn,
                                              (2026, 8), False)
        mock_gen_preview.assert_called_once_with(self.mock_cursor,
                                                 self.mock_conn, (2026, 9),
                                                 35000, False)
    ...
    def test_execute_cli_actions_rollover(
            self, mock_exec_rollover: unittest.mock.MagicMock) -> None:
        ...
        args = argparse.Namespace(list=False,
                                  ...
                                  month=8,
                                  count=35000)
        backend.scripts.game_utils.execute_cli_actions(self.mock_cursor,
                                                       self.mock_conn, args,
                                                       "dummy.ini")
        mock_exec_rollover.assert_called_once_with(self.mock_cursor,
                                                   self.mock_conn,
                                                   "dummy.ini",
                                                   dry_run=True,
                                                   overrides={
