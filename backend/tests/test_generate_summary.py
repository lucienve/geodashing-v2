...
def test_parse_photos_json() -> None:
    ...
    res1 = backend.scripts.generate_summary._parse_photos_json(
...
@unittest.mock.patch("backend.scripts.generate_summary.urllib.request.urlopen",
                     autospec=True)
def test_construct_new_data(mock_urlopen: unittest.mock.MagicMock) -> None:
    ...
    text_empty = "".join(
        [p["text"] for p in result_empty if p["type"] == "text"])
...
@unittest.mock.patch("backend.scripts.generate_summary.os.path.isdir",
                     autospec=True)
@unittest.mock.patch("backend.scripts.generate_summary.os.listdir",
                     autospec=True)
@unittest.mock.patch("backend.scripts.generate_summary.os.path.exists",
                     autospec=True)
@unittest.mock.patch("builtins.open",
                     new_callable=unittest.mock.mock_open,
                     read_data="mock data")
def test_load_chat_history(_mock_open: unittest.mock.MagicMock,
                           mock_exists: unittest.mock.MagicMock,
                           mock_listdir: unittest.mock.MagicMock,
                           mock_isdir: unittest.mock.MagicMock) -> None:
    """Test loading chat history from directory."""
    mock_isdir.return_value = True
    mock_listdir.return_value = [
        'example_1_input.txt', 'example_2_input.txt', 'other.txt'
    ]
...
def test_write_summary_files(mock_file: unittest.mock.MagicMock) -> None:
    ...
    mock_file.assert_any_call("/out/game_123_output.html",
                              'w',
                              encoding='utf-8')
...
@unittest.mock.patch("backend.scripts.generate_summary.google.genai.Client",
                     autospec=True)
def test_get_gemini_client_with_api_key(
        mock_client_class: unittest.mock.MagicMock) -> None:
    ...
    mock_client_class.assert_called_once_with(vertexai=False,
...
def test_get_gemini_client(mock_auth_default: unittest.mock.MagicMock,
                           mock_client_class: unittest.mock.MagicMock) -> None:
    ...
    mock_client_class.assert_called_once_with(
        vertexai=False, credentials=mock_creds.with_quota_project.return_value)


@unittest.mock.patch(
    "backend.scripts.generate_summary.load_system_instructions", autospec=True)
@unittest.mock.patch("backend.scripts.generate_summary.load_chat_history",
                     autospec=True)
def test_generate_summary(mock_load_chat: unittest.mock.MagicMock,
                          mock_load_sys: unittest.mock.MagicMock) -> None:
...
def test_configure_environment_full(
        ...
        mock_exists: unittest.mock.MagicMock) -> None:
...
def test_configure_environment_missing_model(
    ...
    with pytest.raises(ValueError,
                       match="GEMINI_MODEL must be explicitly defined"):
        backend.scripts.generate_summary.configure_environment(
            "/fake/config.ini")
