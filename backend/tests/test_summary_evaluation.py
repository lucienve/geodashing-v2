...
class EvaluationResult(pydantic.BaseModel):
    ...
    passed: bool = pydantic.Field(
        ...,
        description=
        "True if the output meets all qualitative standards (scores >= 4).")
...
class SummaryHTMLValidator(html.parser.HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        self.allowed_tags = {
            ...
            'strong', 'b', 'em', 'i', 'u', 'code', 'pre', 'blockquote', 'ul',
            'ol', 'li', 'a', 'img'
        }
...
def _parse_scores(score_text: str) -> list[tuple[str, int]]:
    ...
    winner_match = re.search(
        r'Winner(?:s \(tied\))?:\s*(.*?)\s+with\s+(\d+)\s+point', score_text)
    if winner_match:
...
def _parse_single_log(
        dp_id: str,
        block_content: str) -> backend.scripts.generate_summary.LogEntry:
    ...
    photos_section_match = re.search(
        r'Photos:\n(.*?)(?=\n\n|\n[A-Z0-9-]+ is near|\Z)', rest, re.DOTALL)
    if photos_section_match:
        photos_text = photos_section_match.group(1)
        pairs = re.findall(r'Thumb:\s*([^\s|]+)\s*\|\s*Full:\s*([^\s]+)',
                           photos_text)
...
def _parse_logs(
        file_content: str) -> list[backend.scripts.generate_summary.LogEntry]:
    ...
    for i in range(1, len(log_blocks), 2):
        logs.append(_parse_single_log(log_blocks[i].strip(),
                                      log_blocks[i + 1]))
    return logs
...
def validate_deterministic_rules(html_content: str) -> list[str]:
    ...
    if re.search(r'<br(?!\s*/>)', html_content):
        errors.append(
            "Void tag <br> must be formatted as self-closing: <br />")
    if re.search(r'<hr(?!\s*/>)', html_content):
        errors.append(
            "Void tag <hr> must be formatted as self-closing: <hr />")
    if re.search(r'<img(?![^>]*/>)', html_content):
        errors.append(
            "Void tag <img> must be formatted as self-closing: <img ... />")

    placeholders = ["[Location]", "[Player]", "[Game Title]", "[Number]"]
    for p in placeholders:
        if p in html_content:
            errors.append(
                f"Remnant placeholder token '{p}' found in output summary")

    quotes_matches = re.findall(
        r'<blockquote>(.*?)</blockquote>\s*<p>(.*?)</p>', html_content,
        re.DOTALL)
    for _, attribution in quotes_matches:
        if 'lucienve' in attribution.lower():
            errors.append(
                "User 'lucienve' was featured in a quote or attribution line")

    first_tag_match = re.search(r'^\s*<h2[^>]*>(.*?)</h2>', html_content,
                                re.DOTALL)
    if not first_tag_match:
        ...
        errors.append(
            f"Header <h2> must start with 'Results:': '{first_tag_match.group(1)}'"
        )
...
def evaluate_summary_quality(
        ...
        input_text: str,
        generated_html: str,
        thinking_level: str | None = None) -> EvaluationResult:
...
def _get_eval_config(
        cfg_p: str) -> tuple[dict[str, str | None], google.genai.Client, str]:
...
@pytest.mark.parametrize("prefix",
                         ["example_1", "example_2", "game_1", "game_2"])
def test_evaluate_benchmark_summary(prefix: str) -> None:
    ...
    ai_config, client, judge_model = _get_eval_config(
        os.path.join(c_dir, '../config.ini'))
    ...
    try:
        ...
        inst_p = os.path.join(c_dir,
                              '../../data/summary_system_instructions.txt')
        ...
        assert not html_errors, (f"HTML Rule Violations for {prefix}:\n" +
                                 "\n".join(html_errors))
