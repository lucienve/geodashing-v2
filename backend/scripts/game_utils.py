...
class HTMLFragmentValidator(html.parser.HTMLParser):
    ...
    FORBIDDEN_TAGS = {
        ...
        'iframe', 'object', 'embed', 'applet', 'svg', 'audio', 'video'
    }

    ALLOWED_TAGS = {
        'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'div', 'span', 'br', 'hr', 'strong',
        'b', 'em', 'i', 'u', 'code', 'pre', 'blockquote', 'ul', 'ol', 'li',
        'a', 'img'
    }
    ...
    def _validate_anchor_attributes(
            self, attrs: list[tuple[str, str | None]]) -> None:
        ...
        for attr, val in attrs:
            if attr.lower() == 'href':
                ...
                if not val or not is_valid:
                    self.errors.append(
                        f"Invalid or empty href in <a> tag: '{val}'")
        if not has_href:
            self.errors.append("Anchor <a> tag is missing href attribute")

    def _validate_image_attributes(
            self, attrs: list[tuple[str, str | None]]) -> None:
        ...
        for attr, val in attrs:
            if attr.lower() == 'src':
                ...
                if not val:
                    self.errors.append(
                        "Image <img> tag has an empty src attribute")
                else:
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if not self.stack:
            self.errors.append(
                f"Unexpected closing tag: </{tag}> (no opening tag)")
            return
...
def list_games(cursor: mysql.connector.cursor.MySQLCursor) -> None:
    ...
    games = typing.cast(
        list[tuple[int, str, datetime.datetime | None,
                   datetime.datetime | None, bool]], cursor.fetchall())

    if not games:
        print("No games found in the database.")
        return
    print(
        f"{'ID':<5} | {'Active':<8} | {'Start Time':<20} | {'End Time':<20} | {'Title'}"
    )
    print("-" * 80)
    for g in games:
        ...
        print(
            f"{g_id:<5} | {active_str:<8} | {start_str:<20} | {end_str:<20} | {title}"
        )


def activate_game(cursor: mysql.connector.cursor.MySQLCursor,
    ...
    cursor.execute("UPDATE games SET is_active = TRUE WHERE id = %s",
                   (game_id, ))
...
def upload_summary(cursor: mysql.connector.cursor.MySQLCursor,
                   conn: mysql.connector.connection.MySQLConnection,
                   game_id: int, file_path: str) -> None:
    ...
    if errors:
        print(
            "Validation failed. The following format violations were detected:"
        )
        for err in errors:
            print(f"- {err}")
        sys.exit(1)
    ...
    cursor.execute("UPDATE games SET summary = %s WHERE id = %s",
                   (html_content, game_id))
    conn.commit()
...
def load_mail_config(config_path: str) -> tuple[str, str]:
    ...
    credentials_path = config['mail'].get('GOOGLE_APPLICATION_CREDENTIALS',
                                          '').strip('"\'')
    ...
    if not credentials_path:
        print(
            "Error: GOOGLE_APPLICATION_CREDENTIALS is not configured in config.ini."
        )
        sys.exit(1)
...
def send_via_gmail_api(credentials_path: str, to_email: str, subject: str,
                       html_body: str) -> None:
...
def email_summary(cursor: mysql.connector.cursor.MySQLCursor, game_id: int,
    ...
    cursor.execute(
        "SELECT title, summary, start_time FROM games WHERE id = %s",
        (game_id, ))
    row = typing.cast(tuple[str, str | None, datetime.datetime | None] | None,
                      cursor.fetchone())
    if not row:
        print(f"Error: Game ID {game_id} does not exist.")
        sys.exit(1)
    ...
    if not summary:
        print(f"Error: Game ID {game_id} ('{title}') "
    ...
    if app_env == 'testing':
        print(
            f"APP_ENV=testing: Suppressed physical email transmission to {to_email}"
        )
        return

    print(
        f"Preparing to email summary for Game {game_id} ('{title}') to {to_email}..."
    )

    # Authenticate and send email cleanly
    try:
        month_year = (start_time.strftime("%B %Y")
                      if start_time else "Unknown Month")
...
def load_game_config(config_path: str) -> dict[str, typing.Any]:
    ...
    if 'game' in config:
        ...
        default_config['timezone'] = game_sec.get(
            'TIMEZONE', fallback='America/New_York').strip('"\'')
...
def get_game_title(year: int, month: int, titles_path: str) -> str:
    ...
    if os.path.exists(titles_path):
        try:
            ...
            print(
                f"Warning: Failed to parse titles file at '{titles_path}': {e}. Using fallback."
            )
...
def get_games_for_month(
    cursor: mysql.connector.cursor.MySQLCursor, year: int, month: int
) -> list[tuple[int, str, bool, datetime.datetime | None, datetime.datetime
                | None]]:
    ...
    cursor.execute(
        ...
        "ORDER BY id ASC", (year, month))
    return typing.cast(
        list[tuple[int, str, bool, datetime.datetime | None,
                   datetime.datetime | None]], cursor.fetchall())


def build_turnover_announcement_email(
        active_game: tuple[int, str], active_date: datetime.datetime,
        preview_game: tuple[int, str],
        preview_date: datetime.datetime) -> tuple[str, str]:
...
def send_turnover_announcement(config_path: str,
                               subject: str,
                               html_body: str,
    ...
    if dry_run:
        print(
            f"[DRY RUN] Would send turnover announcement to {to_email} with subject: '{subject}'"
        )
        return
...
def _activate_current_month_game(
        ...
        conn: mysql.connector.connection.MySQLConnection,
        target_date: tuple[int, int], dry_run: bool) -> tuple[int, str]:
    ...
    if is_already_active:
        print(
            f"Current month game (Game {active_id}: '{active_title}') is already ACTIVE."
        )
    else:
        print(
            f"Activating current month game (Game {active_id}: '{active_title}')..."
        )
        if not dry_run:
            cursor.execute("UPDATE games SET is_active = FALSE")
            cursor.execute("UPDATE games SET is_active = TRUE WHERE id = %s",
            ...
            print(
                f"[DRY RUN] Would update games SET is_active = TRUE for ID {active_id}"
            )
...
def _generate_next_preview_game(
        cursor: mysql.connector.cursor.MySQLCursor,
        conn: mysql.connector.connection.MySQLConnection,
        target_date: tuple[int, int], dashpoint_count: int,
    ...
    if next_games:
        ...
        print(
            f"Next month preview game exists (Game {preview_id}: '{preview_title}'). "
            "Skipping generation.")
    ...
    preview_title = get_game_title(
        ...
        os.path.join(current_dir, '../../data/game_titles.json'))
    ...
    if not dry_run:
        points = backend.scripts.generate_game.generate_valid_dashpoints(
            ...
            land_zip_path=os.path.join(current_dir,
                                       '../../data/ne_10m_land.zip'),
            lakes_zip_path=os.path.join(current_dir,
        ...
        backend.scripts.generate_game.bulk_insert_dashpoints(
            cursor, points, preview_id,
            os.path.join(current_dir, '../../data/bad_words.txt'))
        conn.commit()
        print(
            f"Preview Game {preview_id} ('{preview_title}') seeded successfully "
            f"with {len(points)} points.")
    else:
        preview_id = 9999
        print(
            f"[DRY RUN] Would generate {dashpoint_count} dashpoints and insert preview game "
            f"'{preview_title}'.")

    return preview_id, preview_title


def _dispatch_announcement_email(config_path: str,
                                 active_info: tuple[int, str,
                                                    datetime.datetime],
                                 preview_info: tuple[int, str,
                                                     datetime.datetime],
                                 dry_run: bool) -> None:
    ...
    try:
        send_turnover_announcement(config_path,
                                   subject,
                                   html_body,
                                   dry_run=dry_run)
    except Exception as e:
        print(f"ERROR: Failed to dispatch turnover announcement email: {e}",
              file=sys.stderr)
        raise


def _compute_rollover_dates(
    tz_name: str, overrides: dict[str, int | None] | None
    ...
    next_year, next_month = (cur_year + 1,
                             1) if cur_month == 12 else (cur_year,
                                                         cur_month + 1)

    return (datetime.datetime(cur_year, cur_month, 1, tzinfo=tz),
            datetime.datetime(next_year, next_month, 1, tzinfo=tz))


def execute_rollover(cursor: mysql.connector.cursor.MySQLCursor,
                     conn: mysql.connector.connection.MySQLConnection,
                     config_path: str,
                     dry_run: bool = False,
                     overrides: dict[str, int | None] | None = None) -> None:
    ...
    dashpoint_count = count_override or game_config.get(
        'default_dashpoint_count', 35000)
    ...
    if dry_run:
        print(
            "MODE: DRY RUN (No database changes or emails will be dispatched)")
    print("=" * 80)

    # 1. Activate Current Month Game
    active_game = _activate_current_month_game(cursor, conn,
                                               (cur_date.year, cur_date.month),
                                               dry_run)

    # 2. Inspect & Generate Next Month Preview Game
    preview_game = _generate_next_preview_game(
        cursor, conn, (next_date.year, next_date.month), dashpoint_count,
        dry_run)

    # 3. Dispatch Player Announcement Email
    if game_config.get('send_turnover_announcement', True):
        _dispatch_announcement_email(
            config_path, (active_game[0], active_game[1], cur_date),
            (preview_game[0], preview_game[1], next_date), dry_run)
...
def execute_cli_actions(cursor: mysql.connector.cursor.MySQLCursor,
                        conn: mysql.connector.connection.MySQLConnection,
                        args: argparse.Namespace, config_path: str) -> None:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Geodashing Game Administration Utility")
    parser.add_argument('--list',
                        action='store_true',
                        help="List all games chronologically")
    parser.add_argument(
        '--activate',
        type=int,
        metavar='GAME_ID',
    ...
    if (not args.list and args.activate is None and args.upload_summary is None
            and not args.email_summary and not args.rollover):
        parser.print_help()
        sys.exit(1)
