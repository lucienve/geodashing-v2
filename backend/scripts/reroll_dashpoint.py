...
def haversine_distance_km(lat1: float, lon1: float, lat2: float,
                          lon2: float) -> float:
...
def is_valid_land_point(
        lat: float, lon: float,
        spatial_filter: backend.scripts.generate_game.SpatialFilter) -> bool:
    ...
    if 0 in land_map:
        ...
        return spatial_filter.eval_boundary_point(buffer_geom, matching_lands,
                                                  matching_lakes)
...
def generate_polar_candidate(
        ...
        max_radius_km: float = 10.0) -> tuple[float, float]:
    ...
    sample_distance_km = math.sqrt(rand_fraction *
                                   (max_radius_km**2 - min_radius_km**2) +
                                   min_radius_km**2)
    ...
    cand_lat_rad = math.asin(
        ...
        math.cos(origin_lat_rad) * math.sin(angular_distance_rad) *
        math.cos(bearing_rad))
    cand_lon_rad = origin_lon_rad + math.atan2(
        math.sin(bearing_rad) * math.sin(angular_distance_rad) *
        math.cos(origin_lat_rad),
        math.cos(angular_distance_rad) -
        math.sin(origin_lat_rad) * math.sin(cand_lat_rad))
...
def find_valid_reroll_point(
    ...
    for _ in range(max_attempts):
        lat_f, lon_f = generate_polar_candidate(origin, MIN_REROLL_RADIUS_KM,
                                                max_radius_km)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Reroll a dashpoint location on dry land.")
    ...
    parser.add_argument("--max-radius-km",
                        type=float,
                        default=10.0,
                        help="Maximum relocation radius in km")
    parser.add_argument("--land-zip",
                        type=str,
                        default="data/ne_10m_land.zip",
                        help="Path to land shapefile")
    parser.add_argument("--lakes-zip",
                        type=str,
                        default="data/ne_10m_lakes.zip",
                        help="Path to lakes shapefile")
    ...
    try:
        ...
        new_lat, new_lon = find_valid_reroll_point((args.lat, args.lon),
                                                   args.max_radius_km,
                                                   spatial_filter,
